...
async def lifespan(app: FastAPI):
    ...
    if settings.ai_provider == "anthropic" and not settings.anthropic_api_key:
        logger.error(
            "AI_PROVIDER=anthropic but ANTHROPIC_API_KEY is not set: extraction "
            "will fail. Set the key, or switch AI_PROVIDER to rule_based."
        )
    if not settings.api_key:
        logger.warning(
            "API_KEY is not set: /api/v1 is unauthenticated. Acceptable for local "
            "development only; requests are refused outside APP_ENV=development."
        )

    from app.services.scheduler import scheduler as background_scheduler

    background_scheduler.settings = settings
