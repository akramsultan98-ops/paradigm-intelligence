...
from app.services.relevance import assess
...
def process_document(
    ...
    *,
    ...
    # The relevance gate runs before anything is written, so a noise document
    # costs one cheap keyword pass and nothing else — no row, no API call.
    if settings.relevance_enabled:
        verdict = assess(document.title, document.content, settings)
        if not verdict.relevant:
            stats.filtered_irrelevant += 1
            logger.info(
                "document filtered as irrelevant",
                extra={
                    "url": document.url,
    ...
    *,
    ...
    for adapter in adapters:
        try:
            ...
            # Adapters own HTTP clients now, so a run must hand their sockets back.
            adapter.close()
