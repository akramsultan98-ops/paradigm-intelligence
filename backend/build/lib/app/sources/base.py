...
class RawDocument:
    ...
    #: Provenance, carried through to the Source row (spec §37).
...
class SourceConfig:
    ...
    confidence: float = 0.5
    company_hint: str | None = None
    #: Higher runs first. Lets the strongest sources claim a story before a
    #: weaker one does, which matters because the first source to report a
    #: signal is the one it gets attributed to.
    priority: int = 50
    #: Per-host politeness budget for this source.
    rate_limit_per_minute: float = 20.0
    #: Per-run document cap. Falls back to INGEST_MAX_DOCS_PER_SOURCE.
    max_docs: int | None = None
    #: Free-text note, e.g. why a source is disabled. Carried for operators.
    notes: str | None = None
...
class SourceAdapter(ABC):
    ...
    def close(self) -> None:  # noqa: B027 - optional hook, not an abstract method
        """Release any resources the adapter holds (HTTP clients, handles).

        Deliberately concrete and empty: most adapters hold nothing, and forcing
