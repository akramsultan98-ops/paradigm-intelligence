...
class Company(Base):
    ...
    #: Set when this company is a subsidiary of another known company (spec §28).
    #: A link, never a merge: a subsidiary has its own signals and its own
    #: contacts, and collapsing it would lose both.
    ...
    parent: Mapped[Company | None] = relationship(
        remote_side=lambda: [Company.id], back_populates="subsidiaries"
    )
    subsidiaries: Mapped[list[Company]] = relationship(back_populates="parent")
    ...
    __table_args__ = (
        ...
        CheckConstraint("parent_company_id IS NULL OR parent_company_id <> id",
